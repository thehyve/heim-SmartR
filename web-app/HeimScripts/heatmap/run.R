...
main <- function(max_rows = 100, sorting = "nodes", ranking = "mean") {
...
addStats <- function(df, sorting, ranking, max_rows) {
  ...
  if (ncol(df) > 3) {
    ...
    } else {                                  # design matrix is being constructed
    ...
    if (useLimma ) {
      ...
      if (length(rankingScore) == 1 && is.na(rankingScore[1])) {
           stop(paste("Illegal ranking method selected: ",ranking))
        }
    } else {
      cleanUpLimmaOutput()  # Delete json file so that it is not displayed
      rankingScore <-
        apply(measurements, 1, rankingMethod)  # Calculating
    ...
    if (twoSubsets) {
      ...
      markerTable                 <- markerTable[with(markerTable, order(-SIGNIFICANCE)), ]
      markerTable["SIGNIFICANCE"] <- NULL
      markerTable                 <- markerTable[1:min(max_rows, nrow(markerTable)), ]
...
getDEgenes <- function(df) {
  ...
  assign("t_fit", top.fit, envir = .GlobalEnv)
